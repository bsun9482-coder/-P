...
> 审计时测试基线：`python -m pytest` → **195 passed / 1 skipped**（全绿不代表无缺陷，仅说明现有测试未覆盖下述路径）
> 说明：所有缺陷均为代码可证实的问题，未做运行时复现；涉及 LLM/网络/edge-tts 的均已 mock。
> 行号基于审计当日代码，修复前请以当前文件为准复核。
> 状态复核：2026-09-23，当前基线 `pytest` → **195 passed / 2 skipped**（197 用例，2 项因服务未运行 / 数据源缺失跳过）、`ruff check .` 干净。

## 编号说明

本文件使用 `BUG-NN`（大写 + 两位数字）作为缺陷编号，**这是全项目唯一的缺陷编号体系**。

历史遗留：早期分批修复的提交信息与代码注释曾用 `bug #N`（小写）引用另一份清单，
**该清单已不在仓库中**，且与本文编号**部分撞号、含义不同**。例如：

| 编号 | 代码注释曾指 | 本文件指 |
|---|---|---|
| `bug #23` | 语音 WS 一次性票据认证 | BUG-23 · ASR 永久失败被无限重试 |
| `bug #31` | javaguide `source_id` 按标题哈希 | BUG-31 · `_migrate` 非事务执行 |

此外代码侧编号自身也存在一号多义（`bug #31` 同时指"迁移非事务""source_id 哈希""旧浏览器回退"）。
2026-09-23 已把代码中的 `bug #N` 引用全部改为**直接陈述原因**、不再引用编号；
今后新增缺陷只登记到本文件的 `BUG-NN`。
...
## 修复状态总览（2026-09-23 复核）

- 2026-09-01 首轮：修复并验证 29 项（前端已 `npm run build`，后端 pytest **194 passed / 2 skipped**、ruff 通过、浏览器回归通过）。
- 2026-09-23 复核：逐条对照当前代码，把此前**漏记**的 **BUG-03、BUG-20** 补入"已修复"，
  并把**实际已修**却仍标"遗留"的 **BUG-12** 移入"已修复"。
  基线复测：`pytest` **195 passed / 2 skipped**（197 用例）、`ruff check .` 干净。

| 状态 | BUG 编号 |
|---|---|
| ✅ 已修复 | BUG-02、BUG-03、BUG-05、BUG-06、BUG-07、BUG-08、BUG-09、BUG-10、BUG-11、BUG-12、BUG-13、BUG-14、BUG-15、BUG-16、BUG-17、BUG-18、BUG-19、BUG-20、BUG-22、BUG-23、BUG-26、BUG-27、BUG-28、BUG-29、BUG-30、BUG-31、BUG-32、BUG-33、BUG-34、BUG-35、BUG-36、BUG-37 |
| ⏳ 遗留/说明 | BUG-01（跨端并发写锁）、BUG-04（barge-in 线程不可中断）——见下方说明 |
| 📋 待评估 | BUG-21（回声 3 字放行，属设计权衡）、BUG-24（报告隐藏需后端索引配合）、BUG-25（hint 死代码，无用户可见故障） |

**2026-09-23 复核依据（可核对）**：

- BUG-03 已修：`QuestionBankDialog.vue` 的 `load()` 已有 `try/catch` 落入 `error` 状态并渲染错误态 + 重试。
- BUG-12 已修：`voice_ws.py` `_tts()` 中合成在 `async with sem` 内、推送在 `await _await_turn(slot)` 之后，锁粒度已分开。
- BUG-20 已修：`useVoiceCall.js` 的 `doSpeak` 已有 `guard` 幂等护栏，事件与超时只触发一次。
- BUG-01 仍在：`_chat_locks` 仍只在 `routers/session.py` 内使用，`voice_ws.py` 未接入。
- BUG-04 仍在：语音 barge-in 时旧生成线程仍不可中断。
- BUG-21 / BUG-24 / BUG-25 仍在：`voiceUtils.js` 仍是 `length < 3` 放行；`ChatView.vue` 仍用 `m.content === chat.report` 判身份；`useVoiceCall.js` 仍在查找从不入数组的 `role === 'hint'`。

> **关于 BUG-01**：首轮为语音 `_produce` 增加了对 `display_history` 的维护（BUG-07），并用 REST 侧有界 `_chat_locks` 防止内存增长（BUG-32）；真正的"语音与 REST 并发写同一会话"跨端锁需要重构语音生成与 REST 共享同一把 per-user 锁，改动面大、有死锁风险，作为遗留架构项评估。

> **关于 BUG-04**：语音 barge-in 时旧 `asyncio.to_thread` 线程不可中断、继续写 `session.messages`。要求对语音生成链路做线程安全重构，超出安全修复范围，建议单独排期处理。
...
- **根因**：无代际号/取消机制；对比 `stores/chat.js` 已用 `gen + AbortController` 解决同类问题，此处未沿用。
...
首轮按"数据安全 → 功能卡死 → 语音体验 → 纪律守卫"的顺序推进，除下列项外均已修复：

1. **BUG-01（High）**：语音与 REST 跨端并发写同一会话，需两端共享同一把 per-user 锁；改动面大、有死锁风险，建议单独排期。
2. **BUG-04（Med）**：语音 barge-in 时旧生成线程不可中断，需对语音生成链路做线程安全重构。
3. **BUG-24（Low）**：报告气泡隐藏改用后端 `done` 事件携带的报告消息索引，替代前端"内容相等"匹配（需前后端各改一处）。
4. **BUG-21（Low）**：外放场景超短 partial 直接打断的策略，属设计权衡，需产品决策。
5. **BUG-25（Low）**：hint 气泡死代码，清理即可，无用户可见影响。
